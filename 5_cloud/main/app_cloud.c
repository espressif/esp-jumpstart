...
static void handle_delta(const char *data, int len)
{
    ...
    ESP_LOGI(TAG, "Delta: %.*s", len, data);
    int ret = json_parse_start(&jctx, data, len);
    if (ret == OS_SUCCESS) {
        // Enter the "state" object
        if (json_obj_get_object(&jctx, "state") == OS_SUCCESS) {
            if (json_obj_get_bool(&jctx, "output", &new_state) == OS_SUCCESS) {
                ...
                publish_reported_state(new_state);
            } else {
                ESP_LOGW(TAG, "Delta JSON 'state' does not contain 'output' boolean");
            }
...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event)
{
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED: {
            ...
            publish_reported_state(app_driver_get_state());
            break;
        }
