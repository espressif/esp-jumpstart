...
extern protocomm_security_t protocomm_security1;
                                         ...
                                         SessionData *req, SessionData *resp)
{
    ...
    if (mbed_err != 0) {
        ESP_LOGE(TAG, "Failure at mbedtls_aes_setkey_enc with error code : %d", mbed_err);
        return ESP_FAIL;
    }
    ...
    if (mbed_err != 0) {
        ESP_LOGE(TAG, "Failure at mbedtls_aes_crypt_ctr with error code : %d", mbed_err);
        return ESP_FAIL;
    }
                                         ...
                                         const protocomm_security_pop_t *pop)
{
    ...
    mbed_err = mbedtls_ctr_drbg_seed(ctr_drbg, mbedtls_entropy_func,
                                     entropy, NULL, 0);
    if (mbed_err != 0) {
        ESP_LOGE(TAG, "Failed at mbedtls_ctr_drbg_seed with error code : %d", mbed_err);
        ret = ESP_FAIL;
        goto exit_cmd0;
    }
    ...
    if (mbed_err != 0) {
        ESP_LOGE(TAG, "Failed at mbedtls_ecp_group_load with error code : %d", mbed_err);
        ret = ESP_FAIL;
        goto exit_cmd0;
    }
    ...
    if (mbed_err != 0) {
        ESP_LOGE(TAG, "Failed at mbedtls_ecdh_gen_public with error code : %d", mbed_err);
        ret = ESP_FAIL;
        goto exit_cmd0;
    }
    ...
    mbed_err = mbedtls_mpi_write_binary(&ctx_server->Q.X,
                                        ...
                                        PUBLIC_KEY_LEN);
    if (mbed_err != 0) {
        ESP_LOGE(TAG, "Failed at mbedtls_mpi_write_binary with error code : %d", mbed_err);
        ret = ESP_FAIL;
        goto exit_cmd0;
    }
    ...
    if (mbed_err != 0) {
        ESP_LOGE(TAG, "Failed at mbedtls_mpi_lset with error code : %d", mbed_err);
        ret = ESP_FAIL;
        goto exit_cmd0;
    }
    ...
    flip_endian(cur_session->client_pubkey, PUBLIC_KEY_LEN);
    if (mbed_err != 0) {
        ESP_LOGE(TAG, "Failed at mbedtls_mpi_read_binary with error code : %d", mbed_err);
        ret = ESP_FAIL;
        goto exit_cmd0;
    }
    ...
    if (mbed_err != 0) {
        ESP_LOGE(TAG, "Failed at mbedtls_ecdh_compute_shared with error code : %d", mbed_err);
        ret = ESP_FAIL;
        goto exit_cmd0;
    }
    ...
    if (pop != NULL && pop->data != NULL && pop->len != 0) {
        ...
        if (mbed_err != 0) {
            ESP_LOGE(TAG, "Failed at mbedtls_sha256_ret with error code : %d", mbed_err);
            ret = ESP_FAIL;
            goto exit_cmd0;
        }
    ...
    if (mbed_err != 0) {
        ESP_LOGE(TAG, "Failed at mbedtls_ctr_drbg_random with error code : %d", mbed_err);
        ret = ESP_FAIL;
        goto exit_cmd0;
    }
...
protocomm_security_t protocomm_security1 = {
    .ver = 1,
    .init = sec1_init,
