...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t p_gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    if (event == ESP_GATTS_REG_EVT) {
        if (param->reg.status == ESP_GATT_OK) {
            ...
            ESP_LOGE(TAG, "reg app failed, app_id 0x%02x, status %d",
                     param->reg.app_id,
                     param->reg.status);
            return;
        }
    ...
    case ESP_GATTS_REG_EVT:
        ...
        if (ret) {
            ESP_LOGE(TAG, "create attr table failed, error code = 0x%2x", ret);
            return;
        }
        ret = esp_ble_gap_set_device_name(g_ble_cfg_p->device_name);
        if (ret) {
            ESP_LOGE(TAG, "set device name failed, error code = 0x%2x", ret);
            return;
        }
        ret = esp_ble_gap_config_adv_data(&g_ble_cfg_p->adv_data);
        if (ret) {
            ESP_LOGE(TAG, "config adv data failed, error code = 0x%2x", ret);
            return;
        }
        break;
    ...
    case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
        if (param->add_attr_tab.status != ESP_GATT_OK) {
            ESP_LOGE(TAG, "creating the attribute table failed, error code=0x%02x", param->add_attr_tab.status);
...
esp_err_t simple_ble_start(simple_ble_cfg_t *cfg)
{
    ...
    if (ret) {
        ESP_LOGE(TAG, "%s bt_controller_mem_release failed %d", __func__, ret);
    ...
    if (ret) {
        ESP_LOGE(TAG, "gatts register error, error code = %02x", ret);
        return ret;
    }

    ret = esp_ble_gap_register_callback(gap_event_handler);
    if (ret) {
        ESP_LOGE(TAG, "gap register error, error code = %02x", ret);
        return ret;
    }
    ...
    if (ret) {
        ESP_LOGE(TAG, "gatts app register error, error code = %02x", ret);
        return ret;
    }

    esp_err_t local_mtu_ret = esp_ble_gatt_set_local_mtu(500);
    if (local_mtu_ret) {
        ESP_LOGE(TAG, "set local  MTU failed, error code = %02x", local_mtu_ret);
    }
