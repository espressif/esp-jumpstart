...
def version_match(tp, protover):
    try:
        response = utils.bytes_to_str(tp.send_version_data(utils.str_to_bytes(protover)))
        if response != "SUCCESS":
            return False
        return True
    except RuntimeError as e:
...
def establish_session(tp, sec):
    try:
        ...
        while True:
            ...
            if request == None:
                break
            response = tp.send_session_data(request)
            if (response == None):
                return False
        return True
    except RuntimeError as e:
...
def custom_config(tp, sec, custom_info, custom_ver):
    try:
        ...
        response = tp.send_custom_config_data(message)
        return (prov.custom_config_response(sec, response) == 0)
    except RuntimeErrora as e:
        print(e)
        return None
...
def send_wifi_config(tp, sec, ssid, passphrase):
    try:
        ...
        response = tp.send_config_data(message)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ssid", dest = 'ssid', type = str,
        help= "SSID of wifi newtork", required = True)
    parser.add_argument("--passphrase", dest = 'passphrase', type = str,
        help = "Passphrase of wifi network", default = '')
    ...
    if not version_match(transport, args.protover):
        print("---- Error due to protocol version mismatch ----")
        exit(3)
